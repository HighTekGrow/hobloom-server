...
io.on('connection', (socket) => {
    ...
    emitCurrentCycle(socket);
    socket.emit('appliances', applianceManager.getAppliances());
});

function emitCurrentCycle(socket) {
    if (LightUtils.isLightOnTime(settingsManager)) {
        return socket.emit('current_cycle', { cycle: 'day'});
    }
    return socket.emit('current_cycle', { cycle: 'night'});
}
...
function handleHumidityCheck(humidity) {
    for (var i = 0; i < client_list.length; i++) {
        client_list[i].emit('humidity', humidity);
    }
...
function handleTemperatureCheck(temperature) {
    for (var i = 0; i < client_list.length; i++) {
        client_list[i].emit('temperature', temperature);
    }
